...
2. 将 `backend/config/app.local.production.example.toml` 复制为
   `backend/config/app.local.toml`，并通过服务器 Secret 管理器填写：
   管理员账号密码、会话密钥、客户访问令牌加密密钥、模型连接加密密钥、
   Lark 配置加密密钥和正式 HTTPS 域名。`app.local.toml` 禁止提交到 Git。
3. 创建可写且持久化的目录：
...
- `backend/config/app.local.toml`、`.env` 及其他本地 Secret；
