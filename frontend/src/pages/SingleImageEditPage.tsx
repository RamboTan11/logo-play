    ...
    <ClientShell>
      <main className="client-main single-editor-main">
        ...
        <section className="single-editor-workspace">
          ...
          <section className={'single-editor-canvas' + (isGenerating ? ' is-generating' : '')} aria-busy={isGenerating}><div className="single-editor-canvas-meta"><span>{isGenerating ? t('修改版本生成中') : t('当前版本') + ' V' + activeVersion.number}</span><span>{isGenerating ? t('完成后自动呈现') : activeVersion.editInstruction || t('初始生成')}</span></div><div className={'single-editor-stage' + (isGenerating ? ' is-generating' : '')}>{activeVersion.imageUrl ? <img className="generated-logo-image" src={activeVersion.imageUrl} alt={t('当前 Logo 版本')} /> : <LogoArtwork variant={activeVersion.number - 1} domain={domain} />}{isGenerating && <div className="single-editor-waiting-overlay"><GenerationWaitingState title={t('正在生成修改版本')} description={t('正在按修改指令完成调整，生成结果会自动显示')} /></div>}</div><div className="single-editor-version-note"><b>{isGenerating ? t('当前版本已安全保留') : activeVersion.editInstruction || t('保持当前方向')}</b><span>{isGenerating ? t('生成完成后会自动切换到新版本') : t('仅显示当前版本与紧邻上一版本')}</span></div></section>
